...
const Sidebar: React.FC<SidebarProps> = ({ onLogout, isOnline, onRetry }) => {
  ...
  return (
    <>
      {/* --- DESKTOP SIDEBAR --- */}
      <aside className="hidden md:flex fixed top-0 left-0 h-full w-64 bg-white shadow-xl z-50 flex-col">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 z-[100] px-2 py-1 pb-safe shadow-[0_-4px_12px_rgba(0,0,0,0.05)]">
        <div className="flex justify-around items-center">
          {menuItems.map((item) => {
            ...
            return (
              <Link
                key={item.path}
                to={item.path}
                className={`flex flex-col items-center p-2 min-w-[64px] transition-all ${isActive ? 'text-indigo-600' : 'text-slate-400'}`}
              >
                <svg className={`w-6 h-6 mb-1 ${isActive ? 'scale-110' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <button 
            ...
            className={`flex flex-col items-center p-2 min-w-[64px] ${isMenuOpen ? 'text-indigo-600' : 'text-slate-400'}`}
          >
            <svg className="w-6 h-6 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16m-7 6h7" />
            </svg>
            <span className="text-[10px] font-bold opacity-70">Lainnya</span>
          </button>
        </div>
      </nav>
      ...
      {isMenuOpen && (
        <>
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[110] md:hidden" onClick={() => setIsMenuOpen(false)} />
          <div className="fixed bottom-0 left-0 right-0 bg-white rounded-t-[2rem] p-8 z-[120] animate-slideUp md:hidden">
            <div className="w-12 h-1.5 bg-slate-200 rounded-full mx-auto mb-6" />
            <div className="space-y-3">
              <Link 
                to="/settings" 
                onClick={() => setIsMenuOpen(false)}
                className="flex items-center space-x-4 p-4 bg-slate-50 rounded-2xl text-slate-700 font-bold"
              >
              ...
              <button 
                onClick={onRetry}
                className="flex items-center space-x-4 p-4 w-full bg-slate-50 rounded-2xl text-slate-700 font-bold"
              >
              ...
              <button 
                onClick={onLogout}
                className="flex items-center space-x-4 p-4 w-full bg-red-50 rounded-2xl text-red-600 font-bold"
              >
            ...
            <button onClick={() => setIsMenuOpen(false)} className="mt-6 w-full py-4 text-slate-400 font-bold uppercase tracking-widest text-xs">Tutup</button>
          </div>
        </>
      )}

      {/* --- MOBILE TOP STATUS BAR --- */}
      <div className="md:hidden fixed top-0 left-0 right-0 h-14 bg-white/80 backdrop-blur-md border-b border-slate-100 z-40 flex items-center justify-between px-4">
        <div className="flex items-center space-x-2">
          <div className="w-7 h-7 bg-indigo-600 rounded-lg flex items-center justify-center text-white font-bold text-xs">K</div>
          <span className="font-black text-slate-800 text-sm tracking-tight uppercase">E-Kas Kelas</span>
        </div>
        <div className={`px-2 py-1 rounded-full text-[9px] font-black uppercase ${isOnline ? 'bg-emerald-100 text-emerald-600' : 'bg-amber-100 text-amber-600'}`}>
          {isOnline ? 'Online' : 'Offline'}
        </div>
      </div>
      
      {/* Spacer for top and bottom bars */}
